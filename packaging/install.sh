...
if [ "${1:-}" = "--uninstall" ]; then
    ...
    rm -f   "$(d "$ICONDIR")/tumbleweed-updater-updates.svg"
    rm -rf  "$(d "$RESDIR")"
...
install -m644 data/icons/tumbleweed-updater-updates.svg "$(d "$ICONDIR")/"

install -d "$(d "$RESDIR")/icons"
install -m644 data/icons/tumbleweed-updater.svg         "$(d "$RESDIR")/icons/"
install -m644 data/icons/tumbleweed-updater-updates.svg "$(d "$RESDIR")/icons/"
